    ...
    public partial class aeIndexing : Form, IMessageFilter
    {
        ...
        private bool MandatoryDocTypeChecking()
        {
            ...
            for (int i = 0; i < imageLst.Items.Count; i++)
            {
                ...
                if (docType == ihConstants.ORDERSMAINCASE_FILE || docType == ihConstants.FINALJUDGEMENTORDER_FILE)
                {
                ...
                if (docType == ihConstants.FINALJUDGEMENTORDER_FILE || docType == ihConstants.ORDERSMAINCASE_FILE)
                {
                    dp[ihConstants.FINALJUDGEMENTORDER_FILE] = true;
                    dp[ihConstants.ORDERSMAINCASE_FILE] = true;
                }
        ...
        public bool UpdateStatus(eSTATES state, Credentials prmCrd, bool pLock)
        {
            ...
            if (Convert.ToInt32(lvwDockTypes.Items[14].SubItems[2].Text) == 0 && Convert.ToInt32(lvwDockTypes.Items[16].SubItems[2].Text) == 0)
            {
                if (exception != "")
                {
                    exception = exception + "||" + "07" + "||" + "08";
                }
                else
                {
                    exception = exception + "07" + "08";
            ...
            try
            {
                ...
                if (e.KeyCode == Keys.F5)
                {
                    ...
                    if (CheckINDEXStatus() == true)
                    {
                        ...
                        {
                            policyLst = lstPolicy;
                            rlst = MessageBox.Show(this, "Mandatory document missing(Order Main Case, Main Petition, Main Petition Annexture,  Vakalatnama, Final Judgment) , do you want to proceed?", "Missing", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                            if (rlst == DialogResult.Yes)
                            {
                                policyLst = lstPolicy;
